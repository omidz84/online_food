        ...
        try:
            ...
            if food_id in cart:
                ...
                if cart[food_id] > food.count:
                    return Response({'msg': 'Quantity requested is more than stock'}, status.HTTP_400_BAD_REQUEST)
            else:
                cart[food_id] = 1
                if cart[food_id] > food.count:
                    return Response({'msg': 'Quantity requested is more than stock'}, status.HTTP_400_BAD_REQUEST)
        except:
            return Response({'msg': 'food not found'}, status.HTTP_404_NOT_FOUND)

        request.session['cart'] = cart
        request.session.modified = True

        return Response({'msg': 'Add to cart'}, status.HTTP_201_CREATED)
...
class RemoveInCartView(GenericAPIView):
    ...
    def post(self, request: Request):
        ...
        try:
            ...
            return Response({'msg': 'Not available in cart'}, status.HTTP_400_BAD_REQUEST)

        request.session['cart'] = cart
        request.session.modified = True

        return Response({'msg': 'Remove in cart'}, status.HTTP_200_OK)
...
class SaveCartView(GenericAPIView):
    ...
    def post(self, request: Request):
        ...
        final_price = {}
        if not cart:
            return Response({'msg': 'The cart is empty'}, status=status.HTTP_400_BAD_REQUEST)
        for food_id, quantity in cart.items():
            try:
                food = Food.objects.get(id=food_id)
...
class ShowOrdersView(GenericAPIView):
    ...
    def post(self, request: Request):
        ...
        # serializers = SaveCartSerializers(carts, many=True)
